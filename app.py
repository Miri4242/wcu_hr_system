...
def to_azerbaijan_time(dt):
    """Datetime objesini Azerbaycan saatine (UTC+4) dönüştürür"""
    if dt is None:
        return None
    # Eğer datetime timezone-aware değilse, UTC olduğunu varsay
...
def format_time_az(dt):
    """Datetime objesini Azerbaycan saatine göre formatlar"""
    if dt is None:
        return 'N/A'
    az_time = to_azerbaijan_time(dt)
    return az_time.strftime('%H:%M:%S')
